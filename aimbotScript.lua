...
MainTab:AddTextbox({
	...
	Callback = function(value)
		local num = tonumber(value)
		if num then aimHeightTolerance = num end
	end
})
...
local function shootAt(pos)
	...
	local gun = viewmodel:FindFirstChildWhichIsA("Model") or viewmodel:FindFirstChildWhichIsA("Tool")
	if not gun then return end

	local args = {
		{
			{
				workspace.Map.Borders:FindFirstChild("Part") or workspace,
				pos,
				Vector3.new(0, 0, -1)
			}
		},
	...
	ReplicatedStorage:WaitForChild("Events"):WaitForChild("Shoot"):FireServer(unpack(args))
end
...
RunService.RenderStepped:Connect(function()
	...
	-- Aimbot + Shoot Anywhere
	if aimbotEnabled or shootAnywhere then
